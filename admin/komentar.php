...
while ($rkom =mysql_fetch_array($sqlkom)){
	$sqlus= mysql_query("select * from admin where username = '$rkom[username]'");
	$rus= mysql_fetch_array($sqlus);
 	...
 	echo " <table border= '0' widht='600' cellpadding='5' cellspacing='5'>
			...
			echo "</td> 
			<td><b>$rkom[username]</b><br>$rkom[komentar]<br><i>pada $rkom[tanggalkomentar] WIB <br> <a href=?p=deletekomen&id=$rkom[idkomentar]>Delete </a> </i> </td>
			</td>
			</table>";
